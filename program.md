# autoresearch — Static Cyber-Taoist

This is an experiment to have the LLM do its own research, guided by the cyber-taoist Guard/Break framework.
...
## Cyber-Taoist Decision Framework
...
### Decision Rules (FIXED — do not override)

Apply these rules in order:

1. **SIMPLIFY** — If ΔC > 10% AND the last experiment did not improve: refactor and reduce code complexity before attempting the next experiment.
...
7. strategy chosen: `GUARD`, `BREAK`, or `SIMPLIFY` — use `BASELINE` for the first run
...
**The first run**: Your very first run should always be to establish the baseline, so you will run the training script as is. Record the initial train.py line count for ΔC calculation.

LOOP (up to 200 iterations):

1. Read current `train.py` and `results.tsv`.
2. Compute δ from the last k=10 entries and select strategy (GUARD / BREAK / SIMPLIFY) using the decision rules above.
3. Log your analysis: δ value, S, RR, ID components, ΔC, chosen strategy, and rationale.
...
13. **Every 50 iterations**: Review macro-level direction — is the overall research trajectory productive? What patterns emerge from the δ/strategy/outcome data?
