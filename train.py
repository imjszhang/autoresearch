...
class GPTConfig:
    ...
    rope_theta: float = 500_000.0  # LLaMA-style; default GPT-NeoX uses 10k
...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        # SwiGLU FFN (LLaMA-style), ~same param budget as 4*d ReLU^2 MLP
        h = int(8 * config.n_embd / 3)
        self.hidden_dim = ((h + 127) // 128) * 128
        self.c_fc1 = nn.Linear(config.n_embd, self.hidden_dim, bias=False)
        self.c_fc2 = nn.Linear(config.n_embd, self.hidden_dim, bias=False)
        self.c_proj = nn.Linear(self.hidden_dim, config.n_embd, bias=False)

    def forward(self, x):
        x = self.c_proj(F.silu(self.c_fc1(x)) * self.c_fc2(x))
        return x
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        softcap = 10.934  # micro +0.004 vs 10.93; 10.92 was worse on 8c66d42 line
...
polar_express_coeffs = [
    ...
    (2.3465413258596377, -1.7097828382687081, 0.42323551169305323),  # 6th polar step (repeat last coeff)
    (2.3465413258596377, -1.7097828382687081, 0.42323551169305323),  # 7th polar step (repeat last coeff)
    (2.3465413258596377, -1.7097828382687081, 0.42323551169305323),  # 8th polar step (repeat last coeff)
    (2.3465413258596377, -1.7097828382687081, 0.42323551169305323),  # 9th polar step (repeat last coeff)
]
...
WARMDOWN_RATIO = 0.45   # fraction of time budget for LR warmdown (longer mid-plateau)
FINAL_LR_FRAC = 0.13  # cool-down floor; 0.125 prior best line, slight end-LR bump
...
def build_model_config(depth):
    ...
    assert num_heads % KV_HEAD_GROUP == 0, "n_head must divide by KV_HEAD_GROUP"
    n_kv_head = num_heads // KV_HEAD_GROUP
    return GPTConfig(
