...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        # 3.5× expansion (vs 4×): fewer FFN params → higher steady tok/s under fixed wall budget
...
EMBEDDING_LR = 0.63     # learning rate for token embeddings (Adam); was 0.6
UNEMBEDDING_LR = 0.004  # learning rate for lm_head (Adam)
MATRIX_LR = 0.042       # Muon LR for matrix blocks
SCALAR_LR = 0.52        # learning rate for per-layer scalars (Adam)
WEIGHT_DECAY = 0.2      # cautious weight decay for Muon
ADAM_BETAS = (0.8, 0.926) # BREAK iter169 retry: Adam beta2 0.927→0.926 after MCP-timeout crash
