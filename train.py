...
WINDOW_PATTERN = "L" # full attention all layers (exp: simpler window vs SSSL)

# Optimization
TOTAL_BATCH_SIZE = 2**19 # ~524K tokens per optimizer step
EMBEDDING_LR = 0.75     # learning rate for token embeddings (Adam); exp: +25% vs 0.6
UNEMBEDDING_LR = 0.004  # learning rate for lm_head (Adam)
MATRIX_LR = 0.04        # learning rate for matrix parameters (Muon)
SCALAR_LR = 0.45        # learning rate for per-layer scalars (Adam); exp: 0.5->0.45 (prior 0cca68e win on older baseline)
...
model = torch.compile(model, dynamic=False)  # re-enabled: throughput for 5min budget
